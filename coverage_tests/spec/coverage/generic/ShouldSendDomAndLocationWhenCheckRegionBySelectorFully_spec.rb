...
RSpec.describe 'Coverage Tests' do
  ...
  it 'ShouldSendDomAndLocationWhenCheckRegionBySelectorFully' do
    @driver.get('https://applitools.github.io/demo/TestPages/FramesTestPage/')
    @eyes.configure do |conf|
    conf.app_name = 'Applitools Eyes SDK'
    conf.test_name = 'ShouldSendDomAndLocationWhenCheckRegionBySelectorFully'
    conf.viewport_size = Applitools::RectangleSize.new(700, 460)
  end
    @eyes.open(driver: @driver)
    @driver.execute_script('window.scrollTo(0, 350)')
    @eyes.check(Applitools::Selenium::Target.region(:css, '#overflowing-div').fully)
    result = @eyes.close(false)
    info = get_test_info(result)
    expect(info['actualAppOutput'][0]['image']['location']).to eql({"x" => 10, "y" => 83})
    expect(info['actualAppOutput'][0]['image']['hasDom']).to eql(true)
    dom = get_dom(result, info['actualAppOutput'][0]['image']['domId'])
    expect(dom['childNodes']['1']['childNodes']['3']['attributes']['data-applitools-scroll']).to eql('true')
  end
end
