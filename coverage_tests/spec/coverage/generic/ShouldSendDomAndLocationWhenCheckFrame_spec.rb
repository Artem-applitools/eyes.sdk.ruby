...
RSpec.describe 'Coverage Tests' do
  ...
  it 'ShouldSendDomAndLocationWhenCheckFrame' do
    @driver.get('https://applitools.github.io/demo/TestPages/FramesTestPage/')
    @eyes.configure do |conf|
    conf.app_name = 'Applitools Eyes SDK'
    conf.test_name = 'ShouldSendDomAndLocationWhenCheckFrame'
    conf.viewport_size = Applitools::RectangleSize.new(700, 460)
  end
    @eyes.open(driver: @driver)
    @driver.execute_script('window.scrollTo(0, 350)')
    @eyes.check(Applitools::Selenium::Target.frame(css: '[name="frame1"]'))
    result = @eyes.close(false)
    info = get_test_info(result)
    expect(info['actualAppOutput'][0]['image']['location']).to eql({"x" => 58, "y" => 506})
